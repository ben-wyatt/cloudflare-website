...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const accessGroup = await resolveAccessGroup(env, body.accessCode);
    if (!accessGroup.configured) {
      throw new HttpError("Account creation is not configured yet.", 503, "signup_not_configured");
    }
    if (!accessGroup.groupId) {
    ...
    const count = await db.prepare(
      "SELECT COUNT(*) AS count FROM record_users WHERE group_id = ?",
    ).bind(accessGroup.groupId).first();
    ...
    await db.prepare(
      `INSERT INTO record_users
        (id, username, password_hash, password_salt, password_iterations, created_at, group_id)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
    ).bind(
      userId,
      username,
    ...
    return json({
      user: { id: userId, username, groupId: accessGroup.groupId },
