...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const [listResult, standoutsResult, favoritesResult] = await db.batch([
      db.prepare(
        `SELECT
           ...
           a.release_date_precision AS releaseDatePrecision,
         ...
         ORDER BY lower(a.artist_name), lower(a.name), a.spotify_id`,
      ).bind(user.id, RECORD_SEASON),
      db.prepare(
        `SELECT
           st.review,
         ...
         ORDER BY lower(t.artist_name), lower(t.name), t.spotify_id`,
      ).bind(user.id, RECORD_SEASON),
      db.prepare(
        `SELECT
         ...
         ORDER BY spotify_album_id, spotify_track_id`,
      ).bind(user.id, RECORD_SEASON),
    ]);
    ...
    return json({ season: RECORD_SEASON, items, standouts });
  } catch (error) {
...
export async function onRequestPut({ request, env }) {
  try {
    ...
    const user = await requireUser(env, request);
    const db = requireDb(env);
    const seasonState = user.groupId
      ? await db.prepare(
        `SELECT status, locks_at AS locksAt
         FROM record_group_seasons
         WHERE group_id = ? AND season = ?`,
      ).bind(user.groupId, RECORD_SEASON).first()
      : null;
    const lockTimeReached = seasonState?.locksAt
      && Date.parse(seasonState.locksAt) <= Date.now();
    if ((seasonState?.status && seasonState.status !== "open") || lockTimeReached) {
      throw new HttpError("This season’s album picks are closed.", 409, "ballot_closed");
    }
    const body = await readJson(request);
    if (!Array.isArray(body.items) || body.items.length > MAX_RECORD_ALBUMS) {
      throw new HttpError(
        `A draft can contain up to ${MAX_RECORD_ALBUMS} albums.`,
        400,
        "invalid_list",
      );
    }
    ...
    if (uniqueIds.size !== items.length || items.some((item) => !/^[A-Za-z0-9]+$/.test(item.spotifyId))) {
      throw new HttpError("Each album pick must be a different Spotify album.", 400, "invalid_albums");
    }
    ...
    const standouts = (body.standouts || []).map((item) => {
      const value = item && typeof item === "object" ? item : {};
      return {
        spotifyId: String(value.spotifyId || "").trim(),
        review: String(value.review || "").trim(),
    ...
    const statements = [
      db.prepare("DELETE FROM record_track_favorites WHERE user_id = ? AND season = ?")
        .bind(user.id, RECORD_SEASON),
      db.prepare("DELETE FROM record_list_items WHERE user_id = ? AND season = ?")
        .bind(user.id, RECORD_SEASON),
    ];
      ...
      statements.push(db.prepare(
        `INSERT INTO record_albums
           (spotify_id, name, artist_name, image_url, spotify_url, release_date,
            release_date_precision, album_type, total_tracks, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        album.spotifyUrl,
        album.releaseDate,
        album.releaseDatePrecision || null,
    ...
    for (const item of items) {
      statements.push(db.prepare(
        `INSERT INTO record_list_items
           (user_id, season, spotify_album_id, review, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`,
      ).bind(user.id, RECORD_SEASON, item.spotifyId, item.review, now, now));
    }

    for (const item of standouts) {
      statements.push(db.prepare(
        `INSERT INTO record_standout_tracks
           (user_id, season, spotify_track_id, review, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`,
      ).bind(user.id, RECORD_SEASON, item.spotifyId, item.review, now, now));
    }
      ...
      for (const trackId of item.favoriteTrackIds) {
        statements.push(db.prepare(
          `INSERT INTO record_track_favorites
           ...
           VALUES (?, ?, ?, ?, ?)`,
        ).bind(user.id, RECORD_SEASON, item.spotifyId, trackId, now));
      }
    }
